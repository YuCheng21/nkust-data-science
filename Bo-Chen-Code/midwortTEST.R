setwd("C:/Users/user/Desktop/workspace/master course/data mining/R/20")
getwd()
nc <- c()
num <- 1:20
nc_all <- 0
...
np_gsub <- function(np,i){
  ...
  for (p in 1:nc[i]) {
    ...
    np[is.na(np)]<-""   #處理NA值
    if (np[1,p] != "") {
      count_all_word[i] <- count_all_word[i] + 1 #計算字數
    }
  }
...
#先算各文章字數        加上fileEncoding ="UTF-8"解決亂碼問題
for (i in num) {
...
for (i in num) {
  ...
  #各文章的字數
  #nc[i] <- ncol(read.csv(file, header = FALSE, 
  #                       stringsAsFactors = FALSE, 
  #                       sep = " ",
  #                       fileEncoding ="UTF-8"))
  
  #總字數含標點符號
  #nc_all <- ncol(read.csv(file, header = FALSE, stringsAsFactors = FALSE,sep = " ",fileEncoding ="UTF-8")) + nc_all
  
}

#
#tf_1 <- c()
#for(i in 1:nrow(count_word1)){
#  tf_1[i] <- as.integer(count_word1[i,2])/ ncol(np_1)
#  print(tf_1[i])
#}
#tf_2 <- c()
#for(i in 1:nrow(count_word2)){
#  tf_2[i] <- as.integer(count_word2[i,2])/ ncol(np_2)
#  print(tf_2[i])
#}
